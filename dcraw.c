...
int data_offset, meta_offset, meta_length, nikon_curve_offset; /*UF*/
...
// ColorChecker Chart under 6500-kelvin illumination
  ...
  if (verbose) {
    ...
    FORCC for (j=0; j < 3; j++)
      dcraw_message (DCRAW_VERBOSE, "%c%d", (c | j) ? ',':' ', (int) (best[c][j] * num + 0.5));
...
int CLASS main (int argc, char **argv)
{
  ...
  static char opt, *write_ext, *ofname, *cp;
