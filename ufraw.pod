...
Control how hightlights are restored when applying negative EV.
'clip' restores nothing and is therefore safe from any artifacts.
'lch' resotores in LCH space, resulting in restored highlights with soft
details (good for clouds).
'hsv' resotres in HSV space, resulting in restored hightlights with
sharp details.
The default is 'lch'.
...
'digital' corresponds to using a linear reponse, emulating the
...
recommended not to edit this file. This file is updated from the GUI when
