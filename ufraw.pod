...
=item --interpolation=full|quick|four-color

Interpolation algorithm to use when converting from the bayer-pattern
to normal RGB values. "full" does VNG interpolation that should give
best results but is a bit slow.
"quick" does a bilinear interpolation, which is faster but not as good.

"four-color" is a variation of the VNG interpolation that should only be
used if you see strange square patterns in the
resulting image. See L<http://www.cybercom.net/~dcoffin/dcraw/>.
